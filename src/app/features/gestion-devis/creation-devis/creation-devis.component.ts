...
if (this.isCourtier || this.isBEA) {
  ...
  getAllPack() {
    if (this.route.snapshot.paramMap.get('codeProduit') == '45A') {
      ...
      {
        //FIX ME DELETE THIS
        "dateMiseEnCirculation": moment.utc(this.dateMiseCirculation).add(1,"hour").toISOString(),
...
//console.log("cond",this.formInfosAssure)
    if (this.formInfosAssure.valid) {
      ...
      if (this.typeClient != 'personne physique') {
        ...
        this.devis.dateAssure = moment.utc(this.formInfosAssure.get("dateNaissance").value).add(1,"hour").toISOString()
      ...
      if (this.formParamRisque.valid) {
        ...
        if (((this.codeProduit == '45F' || this.codeProduit == '45L') && !doublonsExist) || this.codeProduit != '45F') {        // this.myStepper.next();
          if (this.route.snapshot.paramMap.get('codeProduit') == "45A") {
            ...
            this.dateMiseCirculation = moment.utc(this.formParamRisque.get("DateDeMiseEnCirculation").value).add(1,"hour").toISOString()
          }
        ...
        let groupeList: any = [];
          ...
          //                 Object.assign(this.risqueConsult, { [param.libelle]: this.formParamRisque.get(paramFormName).value.marque });
          //               } else
          //                 Object.assign(this.risqueConsult, { [param.libelle]: this.formParamRisque.get(paramFormName).value.modele });
              ...
              if (paramFormName == param.formName) {
                ...
                if (param.typeChamp?.description == 'Liste of values' || param.typeChamp?.description == 'Boolean') {
                  ...
                  if (param.typeChamp?.description == 'From Table') {
                    ...
                    description = moment.utc(this.formParamRisque.get(paramFormName).value).add(1,"hour").toISOString();
                    Object.assign(this.risqueConsult, { [param.libelle]: description });
                  }
...
isReadonly(): boolean {
  ...
  onInput(value: any, codeParam: any) {
    ...
    //console.log("prinatge",value ,codeParam)
    //console.log("afficher les multoiRisque ",this.multiRisqueArray)
...
if((codeParam=='P241'||codeParam=='P249') && this.codeProduit=='97')
  {
    ...
    let TotalContenu = this.formParamRisque.get('TotalContenu').value;
    let ValeurContenant = this.formParamRisque.get('ValeurContenant').value;
    //console.log("contenu ,  contenat ===========>",TotalContenu,ValeurContenant)
  ...
  /////// laaaa je reactive pour mettre a jout ValeurTotale

    // Vérifier si les deux valeurs sont numériques avant de faire la somme
    if (!isNaN(ValeurContenant) && !isNaN(TotalContenu)) {
      ...
      if(this.codeProduit=='97' && this.selectedSousProduit=="CTI")
        {
          ///SMMP==50%valuer total cas de industrielle
       this.formParamRisque.get('SMP').setValue(sum*0.5)
       // on ne peut pas le modifier seulment dnzs le cas de habitation et no boUser
...
// //console.log('bonjour ', this.formParamRisque.controls.ValeurContenant)
    ...
    if (this.codeProduit == '95' && codeParam == "P128") {
      this.paraRisqueProduit.filter((param: any) => {
        if (param.codeParam == "P198") {
          ...
          this.formParamRisque.controls[param.formName].setValue(value * this.calculCapitaleMRP)
...
//attt
    if((this.codeProduit== '97' && codeParam=='P245')){
      //console.log("devis ====>",this.devis,this.dataSourceRisque)
      //console.log('je rentre pour update smp')
      this.devis.groupes[0]?.groupeList?.map((rsq:any)=>{
        rsq.risque.paramList.find((param:any)=>param.codeParam=='P245').reponse.description = value
      })
      this.Smp=value
      // this.dataSourceRisque.data.map((rsqs:any)=>rsqs.SMP=value)
